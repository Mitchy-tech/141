...
<?php
	...
	$pdo = new PDO($dsn, $user, $password, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_WARNING));//エラーを警告として表示
	...
	if (!empty($_POST['inputPW']) && $_POST['inputPW']==$Pass) {
	//正しいパスワードが入力されている場合
	...
	} elseif (!empty($_POST['inputPW']) && $_POST['inputPW']!=$Pass) {
	//誤ったパスワードが入力されている場合
